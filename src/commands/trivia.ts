...
module.exports = {
	...
	async execute(interaction) {
		...
		var buttons = new MessageActionRow().addComponents(
			new MessageButton().setCustomId("A").setLabel(incorrect_answers[0]).setStyle("PRIMARY"),

			new MessageButton().setCustomId("B").setLabel(incorrect_answers[1]).setStyle("PRIMARY"),

			new MessageButton().setCustomId("C").setLabel(incorrect_answers[2]).setStyle("PRIMARY"),

			new MessageButton().setCustomId("D").setLabel(incorrect_answers[3]).setStyle("PRIMARY")
		);
		...
		collector.on("end", async (i) => {
			...
			var buttons = new MessageActionRow().addComponents(
				new MessageButton().setCustomId("A").setLabel(incorrect_answers[0]).setStyle(buttonStyleA).setDisabled(true),

				new MessageButton().setCustomId("B").setLabel(incorrect_answers[1]).setStyle(buttonStyleB).setDisabled(true),

				new MessageButton().setCustomId("C").setLabel(incorrect_answers[2]).setStyle(buttonStyleC).setDisabled(true),

				new MessageButton().setCustomId("D").setLabel(incorrect_answers[3]).setStyle(buttonStyleD).setDisabled(true)
			);
